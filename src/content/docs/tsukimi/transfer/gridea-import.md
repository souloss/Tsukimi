...
# Gridea 迁移到 Tsukimi 指南

本指南将帮助您将现有的 Gridea 博客内容迁移到 Tsukimi 主题。
...
## Gridea 与 Tsukimi 文章格式差异

Gridea 使用 Markdown 格式存储文章，并包含 Frontmatter。这使得从 Gridea 迁移到 Tsukimi 相对简单，因为两者都基于 Markdown 和 Frontmatter。
...
将 Gridea 博客源文件目录中的 `posts` 文件夹下的所有 Markdown 文件复制到 Tsukimi 的文章目录 `src/content/posts/`。

### 3. 调整 Frontmatter

这是迁移过程中最关键的一步，您需要根据 Tsukimi 的 Frontmatter 规范调整 Gridea 文章的 Frontmatter。
...
def migrate_gridea_frontmatter(file_path):
    ...
    tsukimi_fm = {}
    tsukimi_fm['title'] = gridea_fm.get('title', 'Untitled')
...
# 示例用法
# posts_dir = 'path/to/your/tsukimi/src/content/posts'
...
Gridea 通常将图片等静态资源存储在 `[源文件目录]/media/` 或文章同级目录下的 `assets` 文件夹中。您需要将这些资源复制到 Tsukimi 的 `public/` 目录下，并更新文章中的路径。

1. **复制资源文件**：将 Gridea 的 `media` 文件夹（或包含图片的 `assets` 文件夹）下的所有图片、视频等媒体文件复制到 Tsukimi 的 `public/images/` 或 `public/assets/` 目录下。

2. **更新文件路径**：在 Markdown 文件中，批量替换图片链接，使其指向 Tsukimi 项目中的正确路径。
   - 示例：将 `![alt](/media/image.jpg)` 替换为 `![alt](/images/image.jpg)`。
   - 如果图片在文章同级目录的 `assets` 文件夹中，例如 `![alt](./assets/image.jpg)`，则需要将其路径调整为 Tsukimi 的相对路径或绝对路径，例如 `![alt](/images/post-slug/image.jpg)` 或 `![alt](/images/image.jpg)`。

### 5. 更新内部链接

如果您的 Gridea 文章中包含指向其他文章或页面的内部链接，请确保这些链接在 Tsukimi 中仍然有效。
...
将 Gridea 迁移到 Tsukimi 相对直接，主要工作集中在 Frontmatter 的字段映射和静态资源路径的调整。通过编写自动化脚本，可以大大提高效率。请务必在迁移后进行彻底的验证，确保所有内容和功能正常。