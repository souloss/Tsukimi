...
本地构建部署是一种灵活的方式，你可以在本地构建 Tsukimi 博客，然后将生成的 dist 文件夹部署到你喜欢的任何平台。这种方式适合对构建过程有完全控制的用户。
...
2. 安装 pnpm 包管理器
3. 克隆或拥有 Tsukimi 博客源码

## 第一步：安装依赖

在你的 Tsukimi 项目根目录下运行：
...
通过这种方式，你可以将 Tsukimi 博客部署到任何支持静态文件的平台，同时保持对整个过程的完全控制。