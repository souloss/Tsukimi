...
- **性能与体验维护**：转场合成提示按生命周期启用，动画属性范围明确；53 个交互图标改为本地打包后，首页减少 13 个第三方请求，弱网和离线状态下的搜索、设置、目录等关键控件不再缺图标；默认 LXGW 字体改为只请求 400/700 两套 CSS，内容站首屏 CSS 再减少 121,399 bytes，回访用户只加载其已保存字体；文件树图标 JSON 延迟到实际文章使用，首页探针节省约 147 KB 本地传输。下一批优先评估核心资源加载顺序和可感知的样式/交互问题。
