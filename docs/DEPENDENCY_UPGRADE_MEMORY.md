...
| 外部字体首屏加载 | 已完成（字重收敛） | LXGW 默认字体只加载 regular/bold；保留 `cdnUrl` 兼容旧配置，后续可再处理已保存字体的首屏提前恢复。 |
...
- 没有选择把完整字体包自托管：`lxgw-wenkai-webfont` 解包约 29.9 MB、`@fontsource` 约 55.9 MB，会明显增加站点部署体积；也没有把样式表改为异步加载，以免当前首屏出现字体交换和布局偏移。后续可在不改动用户并行 `Layout.astro` 的前提下，研究在服务端/HeadTags 阶段读取已保存字体选择，避免回访用户先下载默认 LXGW 再切换。
- 代码提交 `609a1b1` 已推送 `origin/master`；回退该提交即可恢复原 `cdnUrl` 字体样式表加载策略，不影响内容仓库。
...
- **性能与体验维护**：转场合成提示按生命周期启用，动画属性范围明确；53 个交互图标改为本地打包后，首页减少 13 个第三方请求，弱网和离线状态下的搜索、设置、目录等关键控件不再缺图标；默认 LXGW 字体改为只请求 400/700 两套 CSS，内容站首屏 CSS 再减少 121,399 bytes。下一批优先评估已保存字体选择的提前恢复和核心资源加载顺序。
...
- 调整后续优先级：继续处理已保存字体选择的首屏恢复、核心资源加载和可感知样式/交互问题；Live2D 维持低优先级暂缓。
