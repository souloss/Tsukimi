...
| 外部字体首屏加载 | 已完成 | LXGW 默认字体只加载 regular/bold，并在首屏恢复已保存字体；保留 `cdnUrl` 兼容旧配置。 |
| 文件树图标数据按需加载 | 已完成 | 文件树首页不再预载约 458 KB 图标 JSON；仅含文件树的文章异步加载数据，切换和 Swup/迁移后初始化均通过。 |
| 内容站文档指令兼容回归 | 已完成 | 兼容旧式 `::: tabs` 与标题式 tab 内容，更新文档渲染门禁；主站 148 页、内容站 584 页和 26 项浏览器检查通过。 |
| 文档 Pagefind 索引按需加载 | 已完成 | 文档页不再在首屏初始化约 1.5 MB 的搜索索引；首次打开并输入搜索时加载，27 项浏览器门禁通过。 |
...
- 默认字体是 `LXGW WenKai`，其旧 `style.min.css` 会再导入 regular、light、bold 以及三套 mono 字体 CSS；站点实际只使用非 mono 的 400/700 字重。本批次在不改变默认字体视觉身份的前提下，改为直接声明 `lxgwwenkai-regular.css` 和 `lxgwwenkai-bold.css`。
- `FontItem` 新增 `cdnUrls?: string[]`，保留旧的 `cdnUrl?: string`；新增 `getFontStylesheets()` 统一 HeadTags 与运行时字体切换的链接生成和 URL 去重，因此旧主题配置仍可工作，多 CSS 字重配置也不会重复插入。
- 同一内容站首页的字体资源对比：旧链路为 7 个 CDN CSS 请求、181,868 bytes CSS 和 27 个 WOFF2/1,441,612 bytes 字体；新链路为 2 个 CDN CSS 请求、60,469 bytes CSS 和相同的 27 个 WOFF2/1,441,612 bytes 字体。首屏 CSS 减少 5 个请求、121,399 bytes（约 66.7%），没有牺牲实际字形覆盖。
- 主站 Chromium 验证确认 400/700 字重均 `document.fonts.check() === true`，实际加载 400 字重 16 个子集、700 字重 9 个子集；内容站同样加载 400/700，桌面和移动首屏无页面错误。切换 Literata 后只增加 1 个 Google Fonts 链接，重复切换和恢复系统字体均不产生重复 LXGW 链接。
- 主工作区 `pnpm install --offline --frozen-lockfile`、`pnpm check`（359 文件，0 error/warning/hint）、`pnpm type-check`、`pnpm build`（148 页面）和 `pnpm audit --prod`（`No known vulnerabilities found`）通过。独立内容副本 `Tsukimi-Content main@77c4906` 的 `pnpm check`（388 文件，0 error/warning/hint）、`pnpm type-check` 和 584 页面完整构建也通过；内容仓库保持干净。
- 没有选择把完整字体包自托管：`lxgw-wenkai-webfont` 解包约 29.9 MB、`@fontsource` 约 55.9 MB，会明显增加站点部署体积；也没有把样式表改为异步加载，以免当前首屏出现字体交换和布局偏移。后续若 CDN 稳定性成为问题，再单独评估按字符集自托管，而不是直接引入完整字体包。
- HeadTags 增加首个内联启动脚本：读取并校验 `selectedFont`，在 body 解析前设置字体栈并只插入当前字体样式表；无效旧值回退默认字体。生产浏览器验证默认、Literata、系统字体和跨页 Swup 状态均无重复链接或页面错误。
- 代码提交 `609a1b1`、`6647258` 已推送 `origin/master`；回退这两个提交即可恢复原 `cdnUrl` 字体样式表加载策略，不影响内容仓库。

### 文件树图标数据按需加载

- `src/scripts/file-tree.js` 不再静态导入 `folder-map.json` 和 `folder-open-icons.json`；两个约 458 KB 的数据文件改为首次发现文件树时动态加载。
- 首页没有文件树时只保留很小的初始化脚本；包含文件树的文章才请求图标数据。脚本同时覆盖 `DOMContentLoaded`、`astro:page-load` 和 `swup:contentReplaced`，并用节点连接状态和初始化标记避免重复绑定。
- 阻断第三方请求的 Chromium 探针显示，首页本地资源传输由约 532 KB 降至约 386 KB，节省约 147 KB；该收益来自延迟图标数据，不宣称 FCP/LCP 的直接变化。
- 主站 Markdown 文章中的 7 个文件树均完成初始化，展开/收起后的 SVG 正常；无文件树首页不会请求两个 JSON 数据块。内容站文章中的文件树同样通过动态 chunk 加载，浏览器 console/pageerror 均为 0。
- `node --check`、`pnpm check`、`pnpm type-check`、148 页主站构建、内容站 584 页构建和文档浏览器回归均通过；代码提交 `f481a44` 已推送 `origin/master`。
- 后续可评估在布局层进一步按页面标记移除极小的 loader，但当前 `Layout.astro`/`DocsLayout.astro` 正在进行内容兼容改动，暂不扩大变更面。

### 内容站文档指令兼容与渲染回归

- Astro 7 下重新接入 `Tsukimi-Content main@77c4906` 时，发现旧内容使用带空格的 `::: tabs` 和标题式 tab body；`remark-plume-compat` 增加 `tabs` 识别，内容指令处理器兼容 section/heading 两种结构，并保持现代 tab 语法不变。
- 原有文档检查器依赖旧布局和搜索选择器，已改为验证当前 `.docs-layout-container`、文档抽屉、暗色变量、Pagefind 文档过滤、SearchModal、`.md-directive-tabs` 和实际 tab 切换行为，避免把选择器漂移误报成兼容性失败。
- 当前主站构建生成 148 页；独立接入内容仓库生成 584 页，内容仓库 `main@77c4906` 保持干净。主站预览 Chromium 桌面/移动、文档/非文档页面检查共 26 项全部通过，包含零横向溢出、tab 选中状态、搜索结果和 console/pageerror 门禁。
- 代码提交 `b61cd9c` 已推送 `origin/master`；本次只提交兼容层和测试器，内容仓库及用户并行编辑的其他指令/图表文件未被覆盖。

### 文档 Pagefind 索引按需加载

- `DocsLayout.astro` 原先在每个文档页尾部直接调用 `loadPagefindTsukimi()`；移除该调用，保留 loader 供 SearchModal 首次查询使用，避免用户未搜索时初始化文档 Pagefind 运行时和索引资源。
- 产物中的文档 Pagefind 目录约 1.5 MB（包含运行时、WASM、索引和 fragment）；当前文档文章页初始 `PerformanceResourceTiming` 中 `pagefind/tsukimi/pagefind.js` 加载次数为 0，打开搜索并输入 `Docker` 后才加载并返回结果。
- `pnpm check`（359 文件，0 error/warning/hint）、`pnpm type-check`、148 页主站构建和 27 项 Chromium 文档回归均通过；回归覆盖首次不加载、UI 搜索按需加载、docSlug 过滤、移动抽屉、tab 切换和非文档页隔离。
- 代码提交 `f6b1caa` 已推送 `origin/master`。按需路径可能让首次搜索多等待一次网络请求，但搜索场景才承担该成本，首屏和只阅读文档的页面获得更好的资源预算。
...
- **性能与体验维护**：转场合成提示按生命周期启用，动画属性范围明确；53 个交互图标改为本地打包后，首页减少 13 个第三方请求，弱网和离线状态下的搜索、设置、目录等关键控件不再缺图标；默认 LXGW 字体改为只请求 400/700 两套 CSS，内容站首屏 CSS 再减少 121,399 bytes，回访用户只加载其已保存字体；文件树图标 JSON 延迟到实际文章使用，首页探针节省约 147 KB 本地传输；文档 Pagefind 从首屏移到首次搜索，避免只阅读时加载约 1.5 MB 文档索引目录。下一批优先评估核心资源加载顺序和可感知的样式/交互问题。
- **内容兼容性**：Astro 7 主链已验证当前主题与 `Tsukimi-Content` 的 584 页内容；旧式 tab 指令、文档布局、搜索、暗色模式和移动抽屉均纳入 26 项浏览器门禁，避免仅凭主站构建通过就遗漏内容站回归。
- **成本与风险**：锁文件变化较大；Astro 7 暴露并修复了一个旧 Astro 模板语法问题；Markdown-it 15 需要配套 `linkify-it` 6 override；字体仍依赖 jsDelivr，完整自托管会增加约 29.9 MB 以上部署体积；`@swup/astro@1.8.0` 仍是维护瓶颈，目前依靠兼容范围内的传递依赖 override，后续 Swup 适配器升级时应移除这些 override 并重新验证页面转场。
...
- 将 LXGW 默认字体从聚合 `style.min.css` 收敛到 regular/bold 两张 CSS；内容站首屏字体 CSS 请求 7→2、传输 181,868→60,469 bytes，400/700 字重、主站/内容站构建和字体切换验证通过，代码提交 `609a1b1` 已推送。
- 在 HeadTags 首个内联脚本中恢复并校验 `selectedFont`，回访用户不再先下载默认 LXGW；默认、Literata、系统字体、无效存储值和 Swup 跨页状态均验证通过，代码提交 `6647258` 已推送。
- 将文件树的两个图标 JSON 改为按需动态加载；无文件树首页节省约 147 KB 本地传输，主站/内容站文件树展开、Swup 页面替换和重复初始化门禁均通过，代码提交 `f481a44` 已推送。
- 修复内容站旧式 `::: tabs` 与标题式 tab body 的兼容识别，并同步当前文档布局、SearchModal、Pagefind 和暗色模式选择器；主站 148 页、内容站 584 页构建及 26 项浏览器检查通过，代码提交 `b61cd9c` 已推送。
- 将文档 Pagefind 从每页首屏预加载改为首次搜索按需加载；初始资源探针确认索引加载次数为 0，SearchModal 搜索、docSlug 过滤和 27 项文档回归均通过，代码提交 `f6b1caa` 已推送。
- 调整后续优先级：继续处理核心资源加载和可感知样式/交互问题；Live2D 维持低优先级暂缓。
